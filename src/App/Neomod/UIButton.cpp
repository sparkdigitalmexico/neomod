...
void UIButton::draw() {
    ...
    {
        const bool inactive = this->is_loading || !this->bEnabled;
        auto color = inactive ? rgba(0x33, 0x33, 0x33, 0xff) : this->color;

        f32 brightness = inactive ? 1.f : 1.f + (this->fHoverAlpha * 0.2f);
